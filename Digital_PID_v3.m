...
% Last Modified by GUIDE v2.5 26-May-2021 03:11:13
...
set(handles.edit14, 'string','0');%alpha1
set(handles.edit15, 'string','0');%alpha2
%set(handles.edit15, 'string','-0.5');%alpha2
%set(handles.edit14, 'string','-0.5');%alpha1
...
BW=0;
if get(handles.radiobutton1, 'value')%continuous plant
    alpha2=str2num(get(handles.edit15, 'string'));
    alpha1=str2num(get(handles.edit14, 'string'));
...
op2 = bodeoptions;
op.FreqUnits='Hz';
op.Title.FontWeight='Bold';
op.FreqScale='linear';
op2.FreqUnits='Hz';
op2.Title.FontWeight='Bold';
op2.FreqScale='linear';
op.Ylim=[-10,10];
op2.Ylim=[0,20];
bodemag(handles.axes3,c_D,op);
set(handles.axes3, 'XMinorGrid','on', 'YMinorGrid','on');
title(handles.axes3, sprintf('Output Sensitivity (Syp), step disturbance at %g s', t_dist*Ts));

bodemag(handles.axes4,U_D,op2);
set(handles.axes4, 'XMinorGrid','on', 'YMinorGrid','on');
title(handles.axes4, sprintf('Output Sensitivity (Sup), step disturbance at %g s', t_dist*Ts));
...
nP=size(P); nP=nP(2);
if (n-nP<0) % case 1st order & alpha1 and alpha2 = value or not zero
   plot(handles.axes2,0,0);
   plot(handles.axes1,0,0);
   plot(handles.axes3,0,0);
   plot(handles.axes4,0,0);
   h=errordlg('Please when you enter an Auxiliary poles, Change the order of plant denominator to 2nd','Error');
   set(h, 'WindowStyle', 'modal');
   %fprint('error');
   %msg = 'Error occurred.';
   %error(msg);
   %bodemag(handles.axes1,0);
else
